...
int dbflagsdecimal;
bool* flags[12] = {false};
int* flagValues[12] = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048};
...
{
	int result;
	int programStatus, returnValue;
	struct thread *programThread;
	...
	result = sys_waitpid(programThread->pId, &programStatus, 0, &returnValue, 1);
...
static
int
cmd_setdbflag(int nargs, char **args)
{
	...
	char* currentdb = args[1];
	char* status = args[2];
	
	// Convert the on/off status into a boolean, where 1 = on and 0 = off
	int k;
	bool requestOn;
	for (k = 0; status[k] != '\0'; k++)

	if (status[0] == 'o' && status[1] == 'n' && k < 2){
		requestOn = true;
	} else if (status[0] == 'o' && status[1] == 'f' && status[2] == 'f' && k < 3) {
		requestOn = false;
	} else {
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	...
	// change the string given into an integer
	int i = atoi(currentdb);


	// outside of acceptable range of flags -> back to menu
	if(i<1 || i>12){	
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}

	// set integer as array index (indexed 0 to 11)
	i--; 
	
	// For turning flag on/off
	if (!flags[i] && requestOn){
		// turn flag on
		flags[i] = true;
	} else if (flags[i] && !requestOn){
		// turn flag off
		flags[i] = false;
	} else {
	...
	// add all the values of the on flags together
	for (k = 0; k < 12; k++){
		if (flags[k] == true){
			dbflagsdecimal += flagValues[k];
		}
	}
