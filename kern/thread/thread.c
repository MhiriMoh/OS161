...
thread_create(const char *name)
{
	...
	DEBUG(DB_THREADS, "Creating Threads \n");
...
thread_bootstrap(void)
{
	...
	DEBUG(DB_THREADS, "Initializing Threads \n");
	...
	if(processInfoLock == NULL)
		processInfoLock = lock_create("processInfoLock");
	...
	struct processInfo* pThreadInfo = kmalloc(sizeof(struct processInfo));
	if(pThreadInfo == NULL){
		kfree(me);
		kfree(pThreadInfo);
		return NULL;
	...
	pThreadInfo->waitPIdCV = cv_create("waitPIdCV");
	if(pThreadInfo->waitPIdCV == NULL){
		kfree(me);
		kfree(pThreadInfo);
		return NULL;
	}

	pThreadInfo->pId = 1001;
	pThreadInfo->parentPId = 1000;
	pThreadInfo->status = RUNNING;
	pThreadInfo->exitCode = 0;
	pThreadInfo->pThread = me;
	pThreadInfo->next = NULL;
	processListHead = pThreadInfo;
	processListTail = pThreadInfo;
	me->pList = pThreadInfo;
	me->pId = 1001;
...
thread_shutdown(void)
{
	...
	lock_destroy(readLock);
	lock_destroy(processInfoLock);
	    ...
	    struct thread **ret)
{
	...
	//allocate the new process table for this child thread
	struct processInfo* pThreadInfo = kmalloc(sizeof(struct processInfo));
	//if there is no memory free up and retunr error
	if(pThreadInfo == NULL){
		kfree(newguy->t_name);
	...
	//create the waitpid cv for this process
	pThreadInfo->waitPIdCV = cv_create("waitPIdCV");
	//if there is no mem free up what was created and return error
	if(pThreadInfo->waitPIdCV == NULL){
		kfree(newguy->t_name);
		kfree(newguy->t_stack);
		kfree(newguy);
	...
	//setup the process info for this thread 
	lock_acquire(processInfoLock);
	pThreadInfo->pId = processListTail->pId + 1;
	pThreadInfo->parentPId = curthread->pId;
	pThreadInfo->status = RUNNING;
	pThreadInfo->exitCode = 0;
	pThreadInfo->pThread = newguy;
	pThreadInfo->next = NULL;
	processListTail->next = pThreadInfo;
	processListTail = pThreadInfo;
	pThreadInfo->waited = NOTWAITED;
	newguy->pList = pThreadInfo;
	newguy->pId = pThreadInfo->pId;
 ...
 fail:
	...
	cv_destroy(pThreadInfo->waitPIdCV);
	kfree(pThreadInfo);
	kfree(newguy->t_stack);
	kfree(newguy->t_name);
	kfree(newguy);
